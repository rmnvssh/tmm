...
def create_routine():
    ...
    response = requests.post(f'{TARPS_SERVICE_URL}routines', json=valid_routine, headers=headers)
    return response.json()

def create_task():
    headers = {'Authorization': f'Bearer {login_token()['accessToken']}', 'Accept': '*/*', 'Content-Type': 'application/json'}
    taskId = requests.post(f'{TARPS_SERVICE_URL}tasks', json=valid_task, headers=headers)
    return taskId.json()

def create_timeframe():
    headers = {'Authorization': f'Bearer {login_token()['accessToken']}', 'Accept': '*/*', 'Content-Type': 'application/json'}
    response = requests.post(f'{TARPS_SERVICE_URL}timeframes', json=valid_timeframe, headers=headers)
    return response.json()
