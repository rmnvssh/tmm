...
def test_get_timeframe():
    ...
    r = requests.get(f'{TARPS_SERVICE_URL}timeframes/{new_timeframe['id']}', headers=headers)
    response = Response(r)
    print(response)
...
def test_create_timeframe():
    headers = {'Authorization': f'Bearer {login_token()['accessToken']}', 'Accept': '*/*'}
    r = requests.post(f'{TARPS_SERVICE_URL}timeframes', json=valid_timeframe, headers=headers)
    response = Response(r)
    print(response)
    response.assert_status_code(200).validate(Timeframe)
...
def test_update_timeframe():
    headers = {'Authorization': f'Bearer {login_token()['accessToken']}', 'Accept': '*/*'}
    new_timeframe = create_timeframe()
    r = requests.put(f'{PORT_SERVICE_URL}timeframes/{new_timeframe['id']}', json=update_valid_timeframe, headers=headers)
    response = Response(r)
    print(response)
    response.assert_status_code(200).validate(Timeframe)
...
def test_delete_timeframe():
    headers = {'Authorization': f'Bearer {login_token()['accessToken']}', 'Accept': '*/*'}
    new_timeframe = create_timeframe()
    response = requests.delete(f'{PORT_SERVICE_URL}timeframes/{new_timeframe['id']}', headers=headers)
    print(response.url)
    assert response.status_code == 200
    check = requests.get(f'{PORT_SERVICE_URL}timeframes/{new_timeframe['id']}', headers=headers)
