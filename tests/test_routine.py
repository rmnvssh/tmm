...
def test_update_routine():
    ...
    new_routine = create_routine()
    r = requests.put(
        f"{PORT_SERVICE_URL}routines/{new_routine['id']}",
...
def test_mark_routine():
    ...
    new_routine = create_routine()
    r = requests.put(
        f"{PORT_SERVICE_URL}routines/{new_routine['id']}/mark-as-ended", headers=headers
    )
    response = Response(r)
    print(response)
    response.assert_status_code(200).validate(Routine)
...
def test_delete_routine():
    ...
    response = requests.delete(
        f"{PORT_SERVICE_URL}routines/{new_routine['id']}", headers=headers
    )
