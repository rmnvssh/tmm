...
def test_get_task_list():
    headers = {'Authorization': f'Bearer {login_token()['accessToken']}', 'Accept': '*/*'}
    r = requests.get(f'{TARPS_SERVICE_URL}tasks', params={'offset': 0, 'limit': 10}, headers=headers)
    response = Response(r)
    print(response)
    response.assert_status_code(200).validate(Task)
...
def test_update_task():
    headers = {'Authorization': f'Bearer {login_token()['accessToken']}', 'Accept': '*/*'}
    new_task = create_task()
    r = requests.put(f'{PORT_SERVICE_URL}tasks/{new_task}', json=update_valid_task, headers=headers)
    response = Response(r)
    print(response)
    response.assert_status_code(200).validate(Task)
